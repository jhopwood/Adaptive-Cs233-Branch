...
class BitShiftAndMaskProblem(BaseHandler):
	...
	def generate_expression(self, level, ops):
		"""
		...
		if level == 0:
			...
			lhs = self.generate_expression(level-1, ops)
		if op == "<<" or op == ">>":
	...
	def data_for_c2n(self):
		...
		operand = hex(self.generator.randint(0, 4294967295)).replace("L", "")
		...
		if operation == "lshift":
			...
			while (int(mask, 16) << shamt) & 0xffffffff == 0:
				mask = self.generate_mask()
		rdict = {"operand": operand, "mask": mask, "shamt": shamt, "ops": ops, "operation": operation}

		if self.level >= 1:
			# Pick another 32-bit operand and throw it in. 
			rdict["operand2"] = hex(self.generator.randint(0, 4294967295)).replace("L", "")
	...
	def score_c2n(self, question_data, student_answer):
		...
		correct_answer = hex(correct_answer).replace("L", "")
	...
	def data_for_question(self, question_type):
		if question_type == "c2n":
			...
			nesting_level = self.generator.randint(0,3)
			ops = [">>","<<","&","|"]
			expression = self.generate_expression(nesting_level, ops)
