...
function Workspace(canvas) {
	...
	this.draw = function () {
		...
		if (qtype === "mux") {
			var num_inp_select = {3: [2, 1], 6: [4, 2], 11: [8, 3]};
			var terminals = ["a", "b", "c", "d", "e", "f", "g", "h", "x", "y", "z"];
			var input_labels = ["d0", "d1", "d2", "d3", "d4", "d5", "d6", "d7"];
			var select_labels = ["s0", "s1", "s2"];
			var num_inputs = num_inp_select[num_terminals][0];
			var num_select = num_inp_select[num_terminals][1];
			var inputs = {};
			for (var i = 0; i < num_inputs; i++) {
				inputs[terminals[i]] = input_labels[i];
			}
			for (i = 0; i < num_select; i++) {
				inputs[terminals[num_inputs+i]] = select_labels[i];
			}
		}
		for (label in this.inputs) {
			if (qtype === "mux") {
				context.fillText(inputs[label], 10, this.inputs[label].y-10);
	...
	this.click = function (e, x, y) {
		...
		for (var i = 0 ; i < this.factories.length ; i ++) {
			if (this.children[i].contains(x, y)) {
				if (problem_type === "digital_logic") {
					var gate = new this.factories[i](this, (i+.5)*canvas.width/(this.factories.length)-40, 5);
				} else {
					var gate = new this.factories[i](this, (i+1.5)*canvas.width/(this.factories.length+1)-40, 5);
				}
				drag(gate, e);
				return;
			}
	...
	if (problem_type === "digital_logic") {
		var factories = [[And, Or, Not], [Nand], [Nand, Not], [Nor], [Nor, Not]]
		if (family >= 1 && family <= 5) {
			this.factories = factories[family-1];
		}
		for (var i = 0; i < this.factories.length; i++) {
			new this.factories[i](this, (i+.5)*canvas.width/(this.factories.length)-40, 5);
		}
	...
	} else if (problem_type === "build") {
		this.factories = [Mux_2_to_1];
		if (level === 1) {
			this.factories.push(Mux_4_to_1);
		}
		for (var i = 0; i < this.factories.length; i++) {
			// Muxes are moved to the right to maximize workspace
			new this.factories[i](this, (i+1.5)*canvas.width/(this.factories.length+1)-40, 5);
/*			for (var j = 0; j < factory.inputs.length; j++) {
				context.fillText(factory.inputs[j].label, (i+1.5)*canvas.width/(this.factories.length)-3, j*factory.height/factory.inputs.length+5);
			}*/
		}
		if (qtype === "mux") {
			var inputs = ["a", "b", "c", "d", "e", "f", "g", "h", "x", "y", "z"];
			this.inputs = {}
			for (i = 0; i < num_terminals; i++) {
				this.inputs[inputs[i]] = new Connection(10, (i+.5)*(canvas.height-50)/(num_terminals+1)+75, 1, this);
			}
		}
	}
...
function Gate() {
	...
	this.release = function () {
		if (((this.x + this.width) < 0) || ((this.y + this.height) < 0) ||
			(this.x >= this.canvas.width) || (this.y >= this.canvas.height)) {
...
function Nand(workspace, x, y) {
	this.initGate(workspace, x, y, 2);
	this.text = "NAND";
	this.image = new Image();
	this.image.src = "/img/" + this.text + ".png";
	this.operator = "^"
...
Nor.prototype = new Gate();

function Mux_2_to_1(workspace, x, y) {
	this.workspace = workspace;
	workspace.children.push(this);
	this.canvas = workspace.canvas;
	this.width = 35;
	...
	this.inputs = new Array();
	for (var i = 0 ; i < num_inputs; i ++) {
		this.inputs.push(new Connection(2, (i+1)*(this.height/(num_inputs)), 0, this));
		this.inputs[i].label = String(i);
	}
	this.inputs[num_inputs-1].x = this.width/2;
	this.inputs[num_inputs-1].y = .95*this.height;
	this.inputs[num_inputs-1].label = "select0"
	...
	this.value = function () {
		var output = "x";
		var select = this.inputs[this.inputs.length-1].wires[0].value();
		if (select === "x") {
			return "x";
		} else if (select === 0) {
			output = this.inputs[0].wires[0].value();
		} else if (select === 1) {
			output = this.inputs[1].wires[0].value();
		}
		return output;
	}
	...
	this.exp = function () {
		...
		for (var i = 0 ; i < this.inputs.length-1 ; i ++) {
			if (this.inputs[i].connected() && this.inputs[this.inputs.length-1].connected()) {
				var sub_exp = this.inputs[i].wires[0].exp();
				var select = this.inputs[this.inputs.length-1].wires[0].exp();
				if (i === 0) {
					this.str += sub_exp + select + "'+";
				}
				else {
					this.str += sub_exp + select;
				}
...
Mux_2_to_1.prototype = new Gate();

function Mux_4_to_1(workspace, x, y) {
	this.workspace = workspace;
	workspace.children.push(this);
	this.canvas = workspace.canvas;
	this.width = 40;
	...
	this.inputs = new Array();
	for (var i = 0 ; i < num_inputs; i ++) {
		this.inputs.push(new Connection(2, (i+1)*(this.height/(num_inputs-1)), 0, this));
		this.inputs[i].label = String(i);
	}
	this.inputs[num_inputs-2].x = this.width/3;
	this.inputs[num_inputs-2].y = .95*this.height;
	this.inputs[num_inputs-2].label = "select0"
	this.inputs[num_inputs-1].x = 2*this.width/3;
	this.inputs[num_inputs-1].y = .85*this.height;
	this.inputs[num_inputs-1].label = "select1"
	...
	this.value = function () {
		var output = "x";
		var select0 = this.inputs[this.inputs.length-2].wires[0].value();
		var select1 = this.inputs[this.inputs.length-1].wires[0].value();
		if (select0 === "x" || select1 == "x") {
			return "x";
		} else if (select0 === 0) {
			if (select1 === 0) {
				output = this.inputs[0].wires[0].value();
				...
				output = this.inputs[2].wires[0].value();
			}
		} else if (select0 === 1) {
			if (select1 === 0) {
				output = this.inputs[1].wires[0].value();
			} else {
				output = this.inputs[3].wires[0].value();
			}
		}
		return output;
	}
	...
	this.exp = function () {
		this.str = "(";
		for (var i = 0 ; i < this.inputs.length-2 ; i ++) {
			if (this.inputs[i].connected() && this.inputs[this.inputs.length-1].connected() && this.inputs[this.inputs.length-2].connected()) {
				var sub_exp = this.inputs[i].wires[0].exp();
				var select0 = this.inputs[this.inputs.length-2].wires[0].exp();
				var select1 = this.inputs[this.inputs.length-1].wires[0].exp();
				if (i % 2 === 0) {
					this.str += sub_exp + select0 + "'";
				}
				else {
					this.str += sub_exp + select0;
				}
...
Mux_4_to_1.prototype = new Gate();

function Mux_8_to_1(workspace, x, y) {
	this.workspace = workspace;
	workspace.children.push(this);
	this.canvas = workspace.canvas;
	this.width = 50;
	...
	this.inputs = new Array();
	for (var i = 0 ; i < num_inputs; i ++) {
		this.inputs.push(new Connection(2, (i+1)*(this.height/(num_inputs-2)), 0, this));
	}
	this.inputs[num_inputs-3].x = this.width/4;
	this.inputs[num_inputs-3].y = .95*this.height;
	this.inputs[num_inputs-2].x = 2*this.width/4;
	this.inputs[num_inputs-2].y = .85*this.height;
	this.inputs[num_inputs-1].x = 3*this.width/4;
	this.inputs[num_inputs-1].y = .75*this.height;

	this.text = "8-to-1";
	this.image = new Image();
	this.image.src = "/img/" + this.text + "_nolabel.png";

	this.value = function () {
		var output = "x";
		var select0 = this.inputs[this.inputs.length-3].wires[0].value();
		var select1 = this.inputs[this.inputs.length-2].wires[0].value();
		var select2 = this.inputs[this.inputs.length-1].wires[0].value();

		if (select0 === "x" || select1 == "x" || select2 == "x") {
			return "x";
		} else if (select0 === 0) {
			if (select1 === 0) {
				output = this.inputs[0].wires[0].value();
	...
	this.exp = function () {
		this.str = "(";
		for (var i = 0 ; i < this.inputs.length-3 ; i ++) {
			if (this.inputs[i].connected() && this.inputs[this.inputs.length-1].connected() && this.inputs[this.inputs.length-2].connected() && this.inputs[this.inputs.length-3].connected()) {
				var sub_exp = this.inputs[i].wires[0].exp();
				var select0 = this.inputs[this.inputs.length-3].wires[0].exp();
				var select1 = this.inputs[this.inputs.length-2].wires[0].exp();
				var select2 = this.inputs[this.inputs.length-1].wires[0].exp();
				if (i % 2 === 0) {
					this.str += sub_exp + select0 + "'";
				}
				else {
